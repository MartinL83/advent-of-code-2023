...
const one = input.split(/\n/).filter(Boolean).reduce((count, row) => {
  const [gameName, gameInput] = row.split(':');
  const [, id] = gameName.split(' ');

  const { red, green, blue } = gameInput.split(/,|;/g).reduce((acc, curr) => {
    const [value, color] = curr.trim().split(' ');
    const nextValue = value / 1;
    const currentValue = acc[color];

    return {
      ...acc,
      [color]: nextValue > currentValue ? nextValue : currentValue
    };
  }, { red: 0, green: 0, blue: 0 });

  const possible = red <= rule.red && green <= rule.green && blue <= rule.blue;
  const idValue = id / 1;

  if (possible) {
    return count + idValue
  }
 ...
 * Challenge 2
 */
const two = input.split(/\n/).filter(Boolean).reduce((count, row) => {
  const [, gameInput] = row.split(':');
